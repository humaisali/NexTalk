...
const Sidebar = ({ rooms, roomsLoading, onSelectRoom, onCreateRoom, onLanguageChange }) => {
  ...
  return (
    <div className="w-72 bg-nt-surface border-r border-nt-border flex flex-col flex-shrink-0 overflow-hidden">
              ...
              return (
                <button key={room._id} onClick={() => onSelectRoom(room)}
                  className={`w-full text-left px-3 py-2.5 rounded-xl flex items-start gap-2.5 transition-all group
                    ${isActive ? 'bg-nt-blue/15 border border-nt-blue/25 shadow-sm' : 'hover:bg-nt-surface2 border border-transparent'}`}>
      ...
      {/* ── Mood Indicator + History (when in a room) ──────────── */}
      {activeRoom && (
      ...
      {/* ── Language selector ───────────────────────────────────── */}
      <div className="border-t border-nt-border px-3 py-3">
        ...
        {user?.language && user.language !== 'en' && (
          <p className="text-xs text-nt-muted/60 mt-1.5 px-1">
            Incoming messages auto-translate to <strong className="text-nt-cyan">{user.language.toUpperCase()}</strong>
          </p>
        )}
      </div>
      ...
      <div className="border-t border-nt-border p-3 flex items-center gap-2.5">
        ...
        <button onClick={logout}
          className="p-1.5 text-nt-muted hover:text-nt-danger hover:bg-nt-danger/10 rounded-lg transition-all" title="Sign out">
          <FiLogOut size={14} />
        </button>
      </div>
    </div>
